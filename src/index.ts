...
const init = async () => {
    const { applyMiddleware, handleCors, handleBodyRequestParsing, handleCompression } = middleware;
    ...
    applyMiddleware([handleCors, handleBodyRequestParsing, handleCompression], router);

    router.use(flash());
    router.use(routes);
    router.listen(process.env.PORT || undefined, () => logger.log('INFO', `Provinyl services started on port ${process.env.PORT}.`));
}
